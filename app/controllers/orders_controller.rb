class OrdersController < ApplicationController
  ...
  def index
    page = params[:page] || 1        
    @all_orders = Order.paginate :page => page, :per_page => 20, :conditions => ["user_id = ?", self.current_user.id.to_s], :order => 'id DESC'
 ...
 def index2
    page = params[:page] || 1        
    @all_sales =  Order.paginate :page => page, :per_page => 20, :conditions => ["seller_id = ?", self.current_user.seller.id.to_s], :order => 'id DESC'
 ...
 def index3
    page = params[:page] || 1        
    @all_sales =  Order.paginate :page => page, :per_page => 20, :conditions => ["seller_id = ? and (status = ? or status = ?)", self.current_user.seller.id.to_s, "closed", "cancelled"], :order => 'id DESC'
  ...
  def index4
    page = params[:page] || 1        
    @all_orders =  Order.paginate :page => page, :per_page => 20, :conditions => ["user_id = ? and status = ?", self.current_user.id.to_s, "closed"], :order => 'id DESC'
  ...
  def sales1_payment_arrived
    ...
    if params[:do] == "confirm"
      ...
      @deliverables = Deliverable.find(:all, :conditions => ["order_id = ?", @order.id.to_s])
